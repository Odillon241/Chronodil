        ...
        .subscribe((status) => {
          ...
          if (status === 'SUBSCRIBED') {
            ...
            if (retryCountRef.current < maxRetries) {
              ...
              console.warn(
                '⚠️ Connexion real-time Tasks en mode dégradé (fonctionnement sans temps réel)',
              )
...
// ============================================
export function useRealtimeHRTimesheets({
  userId,
  enabled = true,
}: {
  userId: string
      ...
      channelRef.current = supabase
        ...
        .on<Record<string, any>>(
          ...
          (payload) => {
            ...
            const hrTimesheetId =
              (payload.new as any)?.hrTimesheetId || (payload.old as any)?.hrTimesheetId
            if (hrTimesheetId) {
              queryClient.invalidateQueries({
