...
// Export des fonctions chat (messages programmés, rappels de messages)
    ...
    await step.run('send-email', async () => {
      return await sendEmail({
        ...
        html: getTransactionalEmailTemplate({
          ...
          preheader: message,
          content: message,
          buttonText: link ? 'Voir les détails' : undefined,
...
export const sendTimesheetReminders = inngest.createFunction(
  ...
  [{ cron: '0 * * * *' }, { event: 'reminder/timesheet.trigger' }],
    ...
    const notificationResults = await step.run('send-reminders', async () => {
      ...
      for (const user of usersWithoutTime) {
        ...
        import('@/lib/notification-helpers')
          .then(({ sendPushNotificationForNotification }) => {
          ...
          try {
            await sendEmail({
              ...
              html: getTransactionalEmailTemplate({
                ...
                content: message,
                alertType: 'info',
                alertTitle: 'Rappel quotidien',
                alertMessage: 'Pensez à saisir vos heures de travail avant la fin de journée.',
                buttonText: 'Saisir mes heures',
                buttonUrl: link,
              }),
            })
          } catch (error) {
            console.error(`[Inngest] Failed to send email, userId: ${user.id}`, error)
          }
        }
...
export const inngestFunctions = [sendEmailNotification, sendTimesheetReminders]
