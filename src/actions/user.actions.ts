...
export const getMyProfile = authActionClient.schema(z.object({})).action(async ({ ctx }) => {
  const { userId } = ctx
    ...
    if (parsedInput.name || parsedInput.avatar) {
      try {
        ...
        if (error) {
          ...
          console.log(
            '[updateMyProfile] Supabase Auth updated successfully:',
...
// Creer un utilisateur (Admin/HR/DIRECTEUR)
export const createUser = authActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    if (authError || !authData.user) {
      throw new Error(
        `Erreur lors de la création du compte: ${authError?.message || 'Utilisateur non créé'}`,
      )
    }
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    if (parsedInput.data.departmentId !== undefined) {
      updateData.departmentId =
        parsedInput.data.departmentId === null ? null : parsedInput.data.departmentId
    }
    ...
    // Synchroniser avec Supabase Auth si le rôle, le nom ou l'email change
    if (parsedInput.data.role || parsedInput.data.name || parsedInput.data.email) {
      try {
        const { createClient } = await import('@supabase/supabase-js')
        const supabaseAdmin = createClient(
          process.env.NEXT_PUBLIC_SUPABASE_URL!,
        ...
        // Récupérer les métadonnées actuelles
        const { data: currentUser } = await supabaseAdmin.auth.admin.getUserById(parsedInput.id)
        const currentMetadata = currentUser?.user?.user_metadata || {}
        ...
        const updatePayload: { user_metadata: Record<string, any>; email?: string } = {
          user_metadata: newMetadata,
        }

        // Mettre à jour l'email également si modifié
        if (parsedInput.data.email && parsedInput.data.email !== targetUser.email) {
          updatePayload.email = parsedInput.data.email
        }

        if (process.env.NODE_ENV === 'development') {
          console.log('[updateUser] Syncing to Supabase Auth:', {
            userId: parsedInput.id.slice(0, 8) + '...',
            fieldsUpdated: Object.keys(newMetadata).filter((k) => k !== 'avatar_url'),
            emailChanged: !!updatePayload.email,
          })
        }
        ...
        if (supabaseError) {
          console.error('[updateUser] Supabase Auth sync failed')
        } else if (process.env.NODE_ENV === 'development') {
          console.log('[updateUser] Supabase Auth updated successfully')
        }
      } catch (error) {
        console.error('[updateUser] Failed to sync with Supabase Auth:', error)
        // Ne pas échouer la requête si la sync échoue
      }
    }
...
export const getMyTeam = authActionClient.schema(z.object({})).action(async ({ ctx }) => {
...
export const getAllUsers = authActionClient.schema(z.object({})).action(async ({ ctx }) => {
  const { userRole } = ctx
...
export const getAllUsersForChat = authActionClient.schema(z.object({})).action(async () => {
  const users = await prisma.user.findMany({
    select: {
      id: true,
      name: true,
...
export const resetUserPassword = authActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    try {
      ...
      const { data: authUser, error: findError } = await supabaseAdmin.auth.admin.getUserById(
        parsedInput.id,
      )
      ...
      console.log('[User Action] Found Supabase User, userId:', authUser.user.id)

      const { error: updateError } = await supabaseAdmin.auth.admin.updateUserById(parsedInput.id, {
