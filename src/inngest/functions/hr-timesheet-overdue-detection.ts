  ...
  async ({ step }) => {
    ...
    for (const missing of missingTimesheets) {
      const result = await step.run(
        `notify-missing-${missing.user.id}`,
        async () => {
              ...
              try {
                await sendEmail({
                  ...
                  subject: isCritical
                    ...
                    : `⚠️ Feuille de temps manquante`,
                  html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                      <h2 style="color: ${isCritical ? "#dc3545" : isWarning ? "#ffc107" : "#C7522B"};">
                        ${isCritical ? "🚨" : isWarning ? "⚠️" : "📋"} Feuille de temps manquante
                      </h2>
                      <p>Bonjour ${user.name},</p>
                      <p>
                        Votre feuille de temps RH pour la semaine du <strong>${weekStart.toLocaleDateString("fr-FR")}</strong>
                        n'a pas encore été créée.
                      </p>
                      <div style="background-color: ${isCritical ? "#f8d7da" : isWarning ? "#fff3cd" : "#d1ecf1"}; border-left: 4px solid ${isCritical ? "#dc3545" : isWarning ? "#ffc107" : "#17a2b8"}; padding: 15px; margin: 20px 0;">
                        <p style="margin: 0;"><strong>${isCritical ? "Action urgente requise" : isWarning ? "Action requise rapidement" : "Action requise"}</strong></p>
                        <p style="margin: 5px 0 0 0;">
                          Cela fait <strong>${daysSinceDue} jours</strong> que la période est passée.
                          ${isCritical ? "Cette situation est critique et nécessite une action immédiate." : ""}
                        </p>
                      </div>
                      <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard/hr-timesheet/new"
                         style="display: inline-block; background-color: ${isCritical ? "#dc3545" : "#C7522B"}; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; margin-top: 10px;">
            ...
            if (isWarning && user.User) {
              ...
              try {
                ...
                try {
                  ...
                  console.error(
                    `Erreur push notification pour manager ${manager.id}:`,
                    pushError
                  );
                }
                ...
                console.error(
                  `Erreur notification manager ${manager.id}:`,
                  error
                );
              }
            }
              ...
              for (const admin of admins) {
                try {
                  ...
                  try {
                    ...
                    console.error(
                      `Erreur push notification pour admin ${admin.id}:`,
                      pushError
                    );
                  }
                  ...
                  console.error(
                    `Erreur notification admin ${admin.id}:`,
                    error
                  );
                }
              }
