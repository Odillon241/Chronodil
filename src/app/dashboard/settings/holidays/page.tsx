...
import { Calendar as CalendarIcon, Plus, Trash2, Flag, Sparkles, PartyPopper } from 'lucide-react'
import { Spinner } from '@/components/ui/spinner'
import { toast } from 'sonner'
import { format, isSameDay, startOfDay, getYear, getMonth } from 'date-fns'
import { fr } from 'date-fns/locale'
import { getHolidays, createHoliday, deleteHoliday } from '@/actions/settings.actions'
...
const gabonHolidaysTemplate = [
  ...
  },
  {
    ...
    month: 4,
    day: 17,
    description: 'Journée internationale des droits de la femme',
    emoji: '👩',
  },
  {
  ...
  },
  {
    ...
    month: 8,
    day: 15,
    description: "Fête de l'Assomption de la Vierge Marie",
    emoji: '✨',
  },
  {
    ...
    month: 8,
    day: 16,
    description: "Célébration de l'indépendance du Gabon (1960)",
    emoji: '🇬🇦',
  },
  {
    name: "Jour de l'Indépendance (J2)",
    month: 8,
    day: 17,
    description: "Célébration de l'indépendance - Jour 2",
    emoji: '🇬🇦',
  },
...
const variableHolidaysByYear: Record<
  number,
  Array<{ name: string; month: number; day: number; description: string; emoji: string }>
> = {
  2025: [
    ...
    await showConfirmation({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (added > 0) {
            toast.success(
              `${added} jour${added > 1 ? 's' : ''} férié${added > 1 ? 's' : ''} ajouté${added > 1 ? 's' : ''}`,
            )
          } else {
        ...
        <div className="space-y-6">
          ...
          <div className="grid lg:grid-cols-5 gap-6">
            ...
            <Card className="lg:col-span-3 overflow-hidden border shadow-none bg-muted/20">
              <CardHeader className="pb-3 border-b bg-background/50">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg font-bold">Liste des jours fériés</CardTitle>
