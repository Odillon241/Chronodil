    ...
    subsections: [
      {
        ...
        anchor: '#info',
        keywords: ['nom', 'prénom', 'email', 'téléphone'],
      },
      {
        title: 'Informations contractuelles',
        anchor: '#contrat',
        keywords: ['horaires', 'congés', 'solde', 'heures', 'semaine'],
      },
      {
        title: 'Sécurité',
        anchor: '#securite',
    ...
    subsections: [
      {
        title: 'Activation',
        anchor: '#activation',
        keywords: ['activer', 'désactiver', 'on', 'off'],
      },
      { title: 'Horaire', anchor: '#horaire', keywords: ['heure', 'horaire', 'quand'] },
      {
        title: 'Jours',
        anchor: '#jours',
        keywords: ['lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi'],
      },
    ],
  },
  {
    ...
    subsections: [
      {
        title: 'Thème',
        anchor: '#theme',
        keywords: ['thème', 'dark', 'sombre', 'light', 'clair', 'mode'],
      },
      {
        title: 'Langue',
        anchor: '#langue',
      ...
      {
        title: 'Accessibilité',
        anchor: '#accessibilite',
        keywords: ['accessibilité', 'contraste', 'taille', 'zoom'],
      },
    ],
  },
  {
    ...
    subsections: [
      {
        ...
        anchor: '#liste',
        keywords: ['liste', 'voir', 'afficher', 'calendrier'],
      },
      { title: 'Ajouter', anchor: '#ajouter', keywords: ['ajouter', 'nouveau', 'créer'] },
      {
        title: 'Initialiser',
        anchor: '#init',
        keywords: ['initialiser', 'année', 'gabon', 'défaut'],
      },
    ],
  },
  {
    ...
    subsections: [
      { title: 'Liste', anchor: '#liste', keywords: ['liste', 'voir', 'afficher'] },
      {
        ...
        anchor: '#ajouter',
        keywords: ['ajouter', 'nouveau', 'créer'],
      },
    ],
  },
  {
    ...
    subsections: [
      {
        ...
        anchor: '#liste',
        keywords: ['liste', 'voir', 'afficher', 'tableau'],
      },
      {
        title: 'Ajouter un utilisateur',
        anchor: '#ajouter',
        keywords: ['ajouter', 'nouveau', 'créer', 'inviter'],
      },
      {
        title: 'Rôles et permissions',
        anchor: '#roles',
        keywords: ['rôle', 'permission', 'accès', 'admin', 'hr'],
      },
    ],
  },
]
...
export default function SettingsLayout({ children }: { children: ReactNode }) {
  ...
  const filteredNavigation = useMemo(() => {
    if (!searchQuery.trim())
      return roleFilteredNavigation.map((item) => ({ ...item, matchedSubsection: null }))
    ...
    return roleFilteredNavigation
      .map((item) => {
        ...
        const matches =
          titleMatch || descriptionMatch || keywordMatch || !!matchedSubsection || allWordsMatch

        return matches ? { ...item, matchedSubsection: matchedSubsection || null } : null
      })
      .filter(Boolean) as ((typeof roleFilteredNavigation)[0] & {
      matchedSubsection: { title: string; anchor: string } | null
    })[]
  ...
  const getActiveItem = () => {
    ...
    if (pathname.startsWith('/dashboard/settings/notifications'))
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-full -m-3 sm:-m-4 lg:-m-6">
          ...
          <nav>
            {filteredNavigation.length > 0 ? (
              filteredNavigation.map((item) => {
                ...
                const isActive = item.href === activeHref || pathname.startsWith(item.href)
      ...
      <div className="flex-1 min-w-0 p-4 sm:p-6 lg:p-8 lg:ml-64 pb-8">{children}</div>
    </div>
  )
}
