...
export default function ProfilePage() {
    ...
    try {
      ...
      if (emailChanged) {
        ...
        if (!emailResult?.data?.success) {
          toast.error(emailResult?.serverError || 'Erreur')
          setIsSaving(false)
          return
        }
      ...
      const result = await updateMyProfile({
        name: data.name,
        email: emailChanged ? data.email : originalEmail,
        avatar: data.avatar,
  ...
  if (!user)
    return (
      <div className="text-center py-12 text-muted-foreground">Impossible de charger le profil</div>
    )
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleSubmitPassword(handlePasswordChange)} className="space-y-4 pt-4">
              ...
              <div key={field.id} className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="••••••••"
                    {...registerPassword(field.id as any)}
                    className="pr-10"
                  />
                  <Button
                    type="button"
            ...
            <div className="flex gap-3 pt-4 border-t">
              ...
              <Button
                type="submit"
                disabled={isChangingPassword || !isPasswordDirty}
                className="flex-1"
              >
