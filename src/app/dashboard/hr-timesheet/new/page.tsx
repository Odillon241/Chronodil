...
import {
  hrTimesheetSchema,
  hrActivitySchema,
  type HRTimesheetInput,
  type HRActivityInput,
...
export default function NewHRTimesheetPage() {
  ...
  const {
    ...
    formState: { errors: _timesheetErrors },
  ...
  const {
    ...
    watch: watchActivity,
    reset: resetActivity,
    formState: { errors: _activityErrors },
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      try {
        ...
        console.error('Erreur chargement profil:', _error)
      }
    }
    loadUserProfile()
  }, [setTimesheetValue])
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.error('Erreur chargement données:', _error)
  ...
  const handleCatalogItemSelect = (catalogId: string) => {
    ...
    if (item) {
      ...
      setActivityValue(
        'activityType',
        item.category === 'CONTROLE ET REPORTING' ? 'REPORTING' : 'OPERATIONAL',
      )
  ...
  const handleAddActivity = (data: HRActivityInput) => {
    ...
    const selectedTask =
      inputMode === 'task' ? availableTasks.find((t) => t.id === selectedTaskId) : null
  ...
  const onSubmitFinal = async (data: HRTimesheetInput) => {
    ...
    try {
      ...
      for (const act of activities) {
        ...
        console.log('📤 Envoi activité:', act.activityName, {
          startDate,
          endDate,
          totalHours: act.totalHours,
        })
      ...
      if (successCount === activities.length) {
        ...
        toast.warning(
          `Feuille créée avec ${successCount}/${activities.length} activités. Erreurs: ${errors.join(', ')}`,
        )
      } else {
        toast.error(
          `Feuille créée mais aucune activité n'a pu être ajoutée. Erreurs: ${errors.join(', ')}`,
        )
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground truncate">
              Remplissez les informations de la semaine
            </p>
          </div>
        </div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {format(watchTimesheet('weekStartDate') as Date, 'dd MMM yyyy', {
                        locale: fr,
                      })}
                    </Button>
                  </PopoverTrigger>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
              <SheetTrigger asChild>
                <Button
                  variant="outline"
                  className="w-full sm:w-auto shadow-sm border-primary/20 hover:bg-primary/5 text-primary"
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v: any) => setActivityValue('status', v)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v: any) => setActivityValue('periodicity', v)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className="w-full justify-start text-left h-9">
                            <CalendarIcon className="mr-2 h-4 w-4" />
                            {format(
                              (watchActivity('startDate') as Date) || new Date(),
                              'dd/MM/yyyy',
                              { locale: fr },
                            )}
                          </Button>
                        ...
                        <PopoverContent className="w-auto p-0">
                          <Calendar
                            mode="single"
                            selected={watchActivity('startDate') as Date | undefined}
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className="w-full justify-start text-left h-9">
                            <CalendarIcon className="mr-2 h-4 w-4" />
                            {format(
                              (watchActivity('endDate') as Date) || new Date(),
                              'dd/MM/yyyy',
                              { locale: fr },
                            )}
                          </Button>
                        ...
                        <PopoverContent className="w-auto p-0">
                          <Calendar
                            mode="single"
                            selected={watchActivity('endDate') as Date | undefined}
            ...
            <Card className="border-dashed border-2 border-border/50">
              <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-6 max-w-sm">
                  Commencez par ajouter des activités pour cette semaine. Vous pouvez lier des
                  tâches existantes ou créer de nouvelles activités.
                </p>
                <Button
                  variant="outline"
