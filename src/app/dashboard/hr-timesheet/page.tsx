...
export default function HRTimesheetPage() {
  ...
  const currentList = useMemo(() => {
    switch (dataView) {
      case "my": return myTimesheets;
      case "pending": return pendingTimesheets;
      case "validated": return validatedTimesheets;
      case "rejected": return rejectedTimesheets;
      default: return myTimesheets;
    }
  ...
  const handleReject = async (id: string) => {
    ...
    await showConfirmation({
      ...
      variant: "destructive",
      onConfirm: async () => {
        const res = await action({ timesheetId: id, action: "reject", comments: "Rejeté via dashboard" });
        if (res?.data) {
