...
import { MessageSquare, Users, FolderKanban, Hash, X, Search, Lock, Sparkles, Send } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[85vh] p-0 overflow-hidden border-none shadow-2xl bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl rounded-2xl flex flex-col">
        ...
        <div className="relative p-6 pb-2 border-b shrink-0">
          ...
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold tracking-tight mb-1">Nouvelle conversation</DialogTitle>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full mt-6">
            ...
            <div className="p-0 flex-1 overflow-hidden flex flex-col relative h-[500px]">
              ...
              {/* Messages Directs */}
              <TabsContent value="direct" className="h-full flex flex-col m-0 data-[state=active]:flex data-[state=inactive]:hidden">
                      ...
                      filteredUsers.map((user) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-sm group-hover:text-primary transition-colors truncate">{user.name}</p>
                            <p className="text-xs text-muted-foreground truncate opacity-80">
                              {user.email}
                            </p>
                          </div>
                          <Badge variant="secondary" className="bg-transparent border-border/60 text-muted-foreground group-hover:bg-background group-hover:text-foreground transition-colors">{user.role}</Badge>
              ...
              {/* Groupes */}
              <TabsContent value="group" className="h-full flex flex-col m-0 data-[state=active]:flex data-[state=inactive]:hidden">
                <div className="p-6 pb-2 space-y-4 shrink-0">
                  <div className="space-y-2">
                    <Label htmlFor="groupName" className="text-xs font-semibold text-muted-foreground uppercase tracking-wider ml-1">Nom du groupe</Label>
                    <Input
                      id="groupName"
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider ml-1">Membres ({selectedUsers.length})</Label>
                    <div className="relative">
                        ...
                        {selectedUsers.map((userId) => {
                          ...
                          return (
                            <motion.div
                              ...
                              key={userId}
                            >
                              <Badge variant="secondary" className="gap-1.5 pl-1 pr-2 py-1 h-7 bg-primary/10 text-primary hover:bg-primary/15 border-transparent">
                        ...
                        {filteredUsers.length === 0 ? (
                          ...
                          filteredUsers.map((user) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <p className={cn("text-sm font-medium truncate transition-colors", selectedUsers.includes(user.id) && "text-primary")}>
                                  {user.name}
                                </p>
              ...
              {/* Projets */}
              <TabsContent value="project" className="h-full flex flex-col m-0 data-[state=active]:flex data-[state=inactive]:hidden">
                <div className="p-4 px-6 border-b bg-muted/20">
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <Input
                    ...
                    {filteredProjects.length === 0 ? (
                      ...
                      filteredProjects.map((project) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-base truncate group-hover:text-primary transition-colors">{project.name}</p>
                            <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-2">
                              <span className="bg-muted px-1.5 py-0.5 rounded text-[10px] font-mono tracking-wide uppercase border">Code: {project.code}</span>
                            </p>
                          </div>
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity text-primary">
              ...
              {/* Canaux */}
              <TabsContent value="channel" className="h-full flex flex-col m-0 overflow-y-auto data-[state=active]:flex data-[state=inactive]:hidden">
                <div className="p-6 space-y-6">
                  {/* Wrapper pour les infos du canal - même style que le dialog createChannel */}
                  <div className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-2">
                        <Label htmlFor="channelName" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground ml-1">Nom du canal <span className="text-destructive">*</span></Label>
                        <div className="relative">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="channelCategory" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground ml-1">Catégorie</Label>
                        <Input
                          id="channelCategory"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="channelDescription" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground ml-1">Description</Label>
                      <Textarea
                  ...
                  <div className="space-y-3 pt-2">
                    <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground ml-1">Membres initiaux</Label>
                    ...
                    {selectedUsers.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {selectedUsers.map((userId) => {
                          ...
                          if (!user) return null;
                          return (
                            <Badge key={userId} variant="secondary" className="gap-1 bg-primary/10 text-primary hover:bg-primary/20">
                              {user.name}
                              <button
                                onClick={() => removeSelectedUser(userId)}
                          ...
                          {filteredUsers.length === 0 ? (
                            ...
                            filteredUsers.map((user) => (
                              ...
                              >
                                ...
                                <span className="text-sm truncate flex-1 opacity-90">{user.name}</span>
                              </div>
                            ))
                          )}
                        </div>
